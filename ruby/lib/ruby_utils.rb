...
class Object
  ...
  def is_scalar?
    [Fixnum, Integer, Float, String, Bignum, FalseClass, TrueClass].include?(self.class)
  end
  def can_be_empty?
    respond_to?(:empty?)
  end
  def is_numeric?
    [Fixnum, Integer, Float, Bignum].include?(self.class)
  end
end 
class Symbol
  def symbol?
...
class Hash
  def isset? key
    !self[key].nil? ?  self[key].is_numeric? ? true : (self[key].can_be_empty? ? !self[key].empty? : self[key].symbol? ? true : self[key] == true) : false
  end
  def areset? *keys
    if keys.array?
      unset = []
      keys.each{|k| unset << k unless self.isset?(k)}
      {:unset => unset, :result => unset.empty?}
    else
      raise "areset expecting array"
    end
  end
  def hash?
    true
  ...
  def unkeyfy!(options = {})
    default = {:recursive => false, 
               :stringify_symbol => false}
    options = default.merge(options)
    t = self.dup
    self.clear
    t.each_pair{|k, value| 
      options[:recursive] && value.hash? && value.unkeyfy!(options)
      options[:stringify_symbol] && value.symbol? && value = value.stringify
